...
# [QUAN TRỌNG] Đã xóa hoàn toàn dòng import lỗi tại đây.
...
# Đảm bảo đường dẫn này trỏ đúng vào thư mục build Angular của bạn
...
# ==========================================
# 4. MOUNT MEDIA THỦ CÔNG (Fix lỗi hiển thị ảnh)
# ==========================================
try:
    db = SessionLocal()
    # Lấy đường dẫn từ DB hoặc mặc định
    real_media_path_str = setting_crud.get_value(db, "save_media") or "/OC-media"
    db.close()
...
async def serve_spa(full_path: str):
    ...
    # 2. Nếu không tìm thấy file -> Trả về index.html
    index_path = CLIENT_DIR / "index.html"
    if index_path.exists():
        return FileResponse(index_path)
